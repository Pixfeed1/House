...
def is_brick_in_opening(brick_x, brick_y, brick_z, brick_width, brick_height, openings):
    ...
    Le bug précédent causait la suppression de toutes les briques avec plusieurs fenêtres.

    Args:
        brick_x (float): Position X de la brique
        brick_y (float): Position Y de la brique
        ...
        openings (list): Liste des ouvertures (fenêtres/portes)
    ...
    for idx, opening in enumerate(openings):
        ...
        # ✅ SÉCURITÉ 5: Debug logging (seulement si DEBUG_MODE global)
        # Note: DEBUG_MODE est défini dans operators_auto.py, pas accessible ici
        # Pour activer logs, décommenter la ligne suivante:
        # if x_inside and y_inside and z_inside:
        #     print(f"[BrickGeometry] Brique supprimée: centre=({brick_center_x:.2f}, {brick_center_y:.2f}, {brick_center_z:.2f}) dans ouverture #{idx}")

        # Vérifier si le centre est dans l'ouverture en 3D
        if x_inside and y_inside and z_inside:
            return True  # Brique dans une ouverture
