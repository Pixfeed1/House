...
class WallGeometryGenerator:
    ...
    def generate_wall_segments(self, floor_plan: FloorPlan) -> List[WallSegment]:
        ...
        # Tolérance plus permissive pour détecter l'adjacence
        ADJACENCY_TOLERANCE = 0.05  # 5cm au lieu de 1cm par défaut
        ...
        for room in floor_plan.placed_rooms:
            ...
            for other in floor_plan.get_adjacent_rooms(room):
                ...
                # Obtenir le bord partagé avec tolérance augmentée
                shared = room.bounds.get_shared_edge(other.bounds, tolerance=ADJACENCY_TOLERANCE)
                if not shared:
                    continue
                ...
                for door in floor_plan.doors:
                    if door.connects(room.id) and door.connects(other.id):
                        ...
                        # door.position est ABSOLUE, start est ABSOLUE
                        door_pos_rel = door.position - start
                        
                        # Validation: s'assurer que la position est dans les limites
                        if door_pos_rel < 0:
                            print(f"[Geometry] ⚠️ Position porte {door.id} négative ({door_pos_rel:.2f}), corrigé à 0.10")
                            door_pos_rel = 0.10
                        elif door_pos_rel + door.width > end - start:
                            max_pos = (end - start) - door.width - 0.10
                            print(f"[Geometry] ⚠️ Position porte {door.id} dépasse le mur, corrigé à {max_pos:.2f}")
                            door_pos_rel = max(0.10, max_pos)
