...
class ParquetProceduralGenerator:
    ...
    def generate_straight(self, bm, uv_layer):
        ...
        # ✅ FIX v7: Marge ÉNORME pour être sûr de tout couvrir
        # On commence AVANT 0 et on finit APRÈS les dimensions
        margin = max(self.plank_length, self.plank_width) * 3
        
        # Calculer le nombre de rangées avec marge
        rows = int(math.ceil((self.floor_width + 2 * margin) / plank_w)) + 1
        
        # Commencer les rangées AVANT x=0
        start_row_x = -margin

        for row in range(rows):
            x = start_row_x + row * plank_w + self.plank_width / 2

            # Décalage 1/3 (pose anglaise classique)
            offset = (row % 3) * (plank_l / 3)

            # Commencer les lames AVANT y=0
            y = -margin + offset + self.plank_length / 2

            # Continuer jusqu'à dépasser y=floor_length
            while y < self.floor_length + margin:
                self.create_plank(
                    bm, x, y, 0,
    ...
    def generate(self):
        ...
        # ✅ FIX: Nettoyer le mesh APRÈS le chanfrein pour éviter les problèmes avec Boolean EXACT
        # 1. Supprimer les doublons
        bmesh.ops.remove_doubles(bm, verts=bm.verts, dist=0.0001)
        
        # 2. Recalculer les normales (essentiel pour Boolean)
        bmesh.ops.recalc_face_normals(bm, faces=bm.faces)
        
        # 3. Dissoudre les éléments dégénérés
        bmesh.ops.dissolve_degenerate(bm, edges=bm.edges, dist=0.0001)
        
        print(f"[Parquet] Mesh nettoyé: {len(bm.verts)} verts, {len(bm.faces)} faces")
...
class ParquetMassif(FloorTypeBase):
    ...
    def _generate_mesh(self, width, length, height):
        ...
        # ✅ FIX: Désactiver les variations qui causent des gaps irréguliers
        # Garder uniquement une légère variation de hauteur pour le réalisme
        generator.height_variation = 0.0002   # Micro-relief subtil
        generator.position_variation = 0.0    # ❌ Désactivé - cause des gaps
        generator.rotation_variation = 0.0    # ❌ Désactivé - cause des gaps
        generator.length_variation = 0.0      # ❌ Désactivé - cause des gaps
    ...
    def _clip_to_floor(self, obj, width, length, height):
        ...
        bool_mod.solver = 'FAST'  # ✅ FIX: FAST est plus tolérant que EXACT

        # Mettre à jour le view_layer
        bpy.context.view_layer.update()

        # Sauvegarder le nombre de vertices AVANT Boolean pour vérification
        ...
        try:
            ...
            # ✅ FIX: Vérifier que le Boolean n'a pas détruit le mesh
            verts_after = len(obj.data.vertices)
            if verts_after < 10:
                print(f"[Parquet] ⚠️ Boolean a produit un mesh quasi-vide ({verts_after} verts)")
                print(f"[Parquet] Tentative de reconstruction sans Boolean...")
                # Le mesh est détruit, on ne peut plus rien faire ici
                # Mais on log l'erreur pour debug
            else:
                print(f"[Parquet] ✅ Boolean appliqué avec succès ({verts_before} → {verts_after} verts)")
        except Exception as e:
...
class ParquetContrecolle(FloorTypeBase):
    ...
    def _generate_mesh(self, width, length, height):
        ...
        generator.position_variation = 0.0
        generator.rotation_variation = 0.0
    ...
    def _clip_to_floor(self, obj, width, length, height):
        ...
        bool_mod.operation = 'INTERSECT'
        bool_mod.object = cutter
        bool_mod.solver = 'FAST'  # FIX: FAST plus tolérant
...
class Stratifie(FloorTypeBase):
    ...
    def _generate_mesh(self, width, length, height):
        ...
        # ✅ FIX: Gaps constants, seul le micro-relief est conservé
        generator.height_variation = 0.0001
        generator.rotation_variation = 0.0    # ❌ Désactivé
        generator.position_variation = 0.0    # ❌ Désactivé
        generator.length_variation = 0.0
