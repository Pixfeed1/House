...
class FloorTypeBase:
    ...
    def _create_plank_floor(self, width, length, height, plank_width, plank_length):
        ...
        try:
            ...
            num_planks_length = int(width / plank_length) + 2  # +2 pour couvrir le décalage
            ...
            for row in range(num_planks_width):
                ...
                # Décalage pour effet réaliste (brickwork pattern)
                x_offset = (row % 2) * (plank_length / 3)

                # ✅ FIX: Commencer à col=-1 pour créer une planche partielle au début
                for col in range(-1, num_planks_length + 1):
                    x_start = col * plank_length + x_offset

                    # Skip si complètement hors zone
                    if x_start >= width:
                        continue
                    if x_start + plank_length <= 0:
                        continue

                    # ✅ FIX: Ajuster si la planche commence avant 0
                    actual_x_start = max(0, x_start)
                    actual_length = min(plank_length - gap, width - actual_x_start)
                    
                    # Réduire la longueur si on a décalé le début
                    if x_start < 0:
                        actual_length = min(actual_length, plank_length + x_start - gap)

                    actual_width = min(plank_width - gap, length - y_start)

                    if actual_length <= 0.001 or actual_width <= 0.001:
                        continue

                    # Créer une planche
                    self._add_plank(bm, actual_x_start, y_start, actual_length, actual_width)
